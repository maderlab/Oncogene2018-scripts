...
### Select top 5% and plot heatmap of normalized counts in SUMO experiments 30 mins
```{r, message=FALSE}
...
regcts <- readRDS("../data/counts-toppeaks-ICI-30.rds")
...
regcts.sel <- regcts[top.peaks$ICI.30.SUMO==1,]
library(edgeR)
exprs <- cpm(assay(regcts.sel), lib.size=offsets)
...
We precomputed DBA object to read in peaksets output from MACS2 as follow. Please refer to the Diffbind R package to format `sample-table.csv` that provides paths to the peak calls from MACS2.
...
##### add peak calling files
# sumo.dba <- dba(sampleSheet=sample.diffbind[sample.diffbind$Factor=="SUMO",-6])
