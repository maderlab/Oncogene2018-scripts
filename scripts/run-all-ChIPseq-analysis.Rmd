...
dba.plotPCA(t30t180.dba.counts,attributes = c(DBA_CONDITION,DBA_TREATMENT), label = DBA_ID, dotSize = 0.8)
```
...
dba.plotPCA(sumo.dba.counts,attributes = c(DBA_CONDITION,DBA_TREATMENT), label = DBA_ID, dotSize = 0.8)
```
...
## peaks whose binding is postively correlated with sample ordering displayed in the heatmap
up.peaks <- rownames(exprs)[bs$up]
str(up.peaks)
## peaks whose binding is negatively correlated with sample ordering displayed in the heatmap
dn.peaks <- rownames(exprs)[bs$dn]
str(dn.peaks)
